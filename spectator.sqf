...
//Modified by TinfoilHate for reasons
	...
	{if (alive _x && _x distance specPen > 250 and _x == player) then {gdtmod_seagull_players = gdtmod_seagull_players + [_x]; }} foreach PlayableUnits;
	...
	while {true} do {
		...
		{if ((alive _x) && (_x distance specPen > 250) && (_x != player) && (vehiclevarname _x != "camman")) then {gdtmod_seagull_players = gdtmod_seagull_players + [_x]; }} foreach playableunits;
		gdtmod_seagull_players_count = count gdtmod_seagull_players - 1;

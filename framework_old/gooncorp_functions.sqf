...
if (player distance lrrp < 100) exitwith {};
if (local player) then {
if (random 1 < .2) then {
player setposASL [(getpos pad1 select 0), (getpos pad1 select 1) + random 4, 15.3];} else {
player setposASL [(getpos pad1_outside select 0), (getpos pad1_outside select 1) + random 20, 15.3];
};
};
};
...
heli1 setposASL [(getpos helipad1 select 0), (getpos helipad1 select 1), 19.3];
heli2 setposASL [(getpos helipad2 select 0), (getpos helipad2 select 1), 19.3];
heli3 setposASL [(getpos helipad3 select 0), (getpos helipad3 select 1), 19.3];

truck1 setposASL [(getpos supportpad1 select 0), (getpos supportpad1 select 1), 19.3];
...
(_this select 0) setposASL [getpos ((_this select 1) select 0), getpos ((_this select 1) select 1), 19.3];
};
...
gooncorp_spreadenemiesgeneric = {// call with ai, player, range
_locations = position (_this select 0);
        ...
        while {true} do {
          ...
          _obj = (vehicle player);
             _pos = getposASL _obj;
               ...
               _alpha = 0.55 + random 0.12;
               _ps22 = "#particlesource" createVehicleLocal _pos;
               _ps22 setParticleParams [["a3\data_f\ParticleEffects\Universal\Universal.p3d", 16, 12, 8, 0], "", "Billboard", 1, 65, [0, 0, 0], _velocity, (_n * 2), 1.175, 1, 1, [75 + (random 46)], [_color + [0], _color + [_alpha], _color + [0], [0,0,0,0]], [1000], 1, 1, "", "", _obj];
               _ps22 setParticleRandom [3, [400 + (random 8), 400 + (random 10), 75], [2, 2, 0], 1, 0, [0, 0, 0, 0.01], 0, 0];
               _ps22 setParticleCircle [0.1, [0, 0, 0]];
               _ps22 setDropInterval 0.025;
